...
| --- | --- | --- | --- |
| Record | 右手拇指和食指指尖触碰，其他三指握拳 | `Fn` 单击 | 录音开 / 关切换 |
| Submit | 右手呈 `OK` 状，拇指和食指指尖触碰，其他三指张开 | `Enter` 单击 | 录音开启时先停录，再延迟后发送 Enter |
...
### 职责
- 将手部姿态观测转成手势候选
- 通过训练好的轻量分类器或等效特征模型输出 record / submit / cancel / no_action 候选
- 输出抽象手势事件，而不是键盘事件
...
如果后续验收继续表明训练模式在真实使用中复杂且不稳定，则可以直接把 runtime 的手势解释层重构回规则模式；这种情况下，训练 / 校准代码应退为非在线依赖，而不是继续作为主识别链路的一部分。
